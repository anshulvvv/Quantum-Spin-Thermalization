...
// {
//     Spins spins(N);
//     // uniform in [–noise_amp, +noise_amp]
//     std::uniform_real_distribution<double> noise_dist(-noise_amp, noise_amp);

//     for (int j = 0; j < N; j++)
//     {
//         // Determine base polar angle: theta on even sites, pi - theta on odd sites
//         double base_theta = (j % 2 == 0) ? theta : (M_PI - theta);

//         // Add a small random perturbation to the tilt angle
//         double dtheta = noise_dist(rng);
//         double actual_theta = base_theta + dtheta;

//         // All spins lie in the x–z plane, so phi = 0
//         double Sx = std::sin(actual_theta);
//         double Sy = 0.0;
//         double Sz = std::cos(actual_theta);
...
Spins initialize_spins(
    ...
    std::mt19937 &rng)
{
    // 1) Compute candidate tilt angles
    auto clamp_asin = [&](double ratio)
    {
        if (std::abs(ratio) <= 1.0)
            return std::asin(ratio);
        return M_PI / 2; // lock to x‐axis if out of bounds
    };
    double theta_uni = clamp_asin(g / (2 * (J + alpha)));
    double theta_af = clamp_asin(g / (2 * (alpha - J)));

    // 2) Compute their energies per spin
    //   E_uni = ½[ (J+α) cos²θ_uni + g sinθ_uni ]
    double E_uni = 0.5 * ((J + alpha) * std::cos(theta_uni) * std::cos(theta_uni) + g * std::sin(theta_uni));
    //   E_af  = ½[ (α−J) cos²θ_af + 2g sinθ_af ]
    double E_af = 0.5 * ((alpha - J) * std::cos(theta_af) * std::cos(theta_af) + 2 * g * std::sin(theta_af));

    bool useAFM = (E_af < E_uni);
    ...
    for (int j = 0; j < N; j++)
    {
        // pick base angle
        double θ = useAFM
                       ? ((j % 2 == 0) ? theta_af : M_PI - theta_af)
                       : theta_uni;
        // add small random tilt‐noise
        θ += noise_dist(rng);

        // all spins in x–z plane
        spins[j][0] = std::sin(θ);
        spins[j][1] = 0.0;
        spins[j][2] = std::cos(θ);
    }
...
double compute_H_ave(const Spins &spins, double J, double h, double g)
{
    ...
    for (int j = 0; j < N; j++)
    {
        ...
        sum += J * spins[j][2] * spins[next][2] + h * spins[j][2] * spins[j][2] + g * spins[j][0];
    }
    return 0.5 * sum;
}
...
void update_spins(Spins &spins, double J, double h, double g, double T)
{
    ...
    for (int j = 0; j < N; j++)
    {
        int prev = (j - 1 + N) % N;
        int next = (j + 1) % N;
        kappa[j] = J * (spins[prev][2] + spins[next][2]) + 2 * h * spins[j][2];
    }
    ...
    // // assume `spins` is std::vector<std::vector<double>> of size N×3
    // for (int j = 0; j < spins.size(); ++j)
    // {
    //     std::cout << "spin[" << j << "] = (";
    //     for (int k = 0; k < spins[j].size(); ++k)
    //     {
    //         std::cout << spins[j][k];
    //         if (k + 1 < spins[j].size())
    //             std::cout << ", ";
                                 ...
                                 double T, double theta, double noise_amp, unsigned int seed)
{
    ...
    Spins spins = initialize_spins(N, J, h, g, noise_amp, rng);
                                     ...
                                     double g, double T, double theta, double noise_amp, double E0)
{
    ...
    for (int l = 0; l <= L; l++)
    {
        ...
        Q_ensemble[l] = (Q_ensemble[l] - E0) / (h * 100) / 6 - (-E0);
    }
    return Q_ensemble;
}
...
int main()
{
    ...
    int L = 20;  // number of driving cycles (you might reduce this for testing)
    double J = 1.0;
    double g = 0.809;
    double h = 0.7045;
    ...
    double ratio = g / (2 * (h - J));
    double theta;
    if (std::abs(ratio) <= 1.0)
    {
        theta = std::asin(ratio);
    }
    else
    {
        // If it “locks” beyond ±1, go to full x-alignment
        theta = M_PI / 2;
    }
